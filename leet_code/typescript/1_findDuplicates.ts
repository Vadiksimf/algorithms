...
function containsNearbyAlmostDuplicate(nums: number[], indexDiff: number, valueDiff: number): boolean {
    ...
    if (!indexDiff && !valueDiff) return false;
...
function containsNearbyAlmostDuplicate1(nums: number[], indexDiff: number, valueDiff: number): boolean {
    if (nums.length <= 1) return false;
    if (!indexDiff && !valueDiff) return false;
